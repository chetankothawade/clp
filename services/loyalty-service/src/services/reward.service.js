...
export const rewardService = {
  ...
  async updateReward(uuid, payload) {
    const reward = await Reward.findOne({ where: { uuid } });
    if (!reward) BaseService.throwError(404, 'error.not_found');
    await reward.update(toRewardAttributes(payload));
    return reward;
  },

  async updateRewardStatus(uuid, status) {
    const reward = await Reward.findOne({ where: { uuid } });
    if (!reward) BaseService.throwError(404, 'error.not_found');
