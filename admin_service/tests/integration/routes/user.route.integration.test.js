...
const updateMyProfile = jest.fn((req, res) => res.status(200).json({ ok: "profile" }));
const changeMyPassword = jest.fn((req, res) => res.status(200).json({ ok: "password" }));
...
describe("user.route integration", () => {
  ...
  it("POST /api/v1/user/create should run upload+validation and call controller", async () => {
    const response = await client.request({
      ...
      body: { first_name: "Neo", last_name: "Anderson", email: "neo@example.com" },
    });
