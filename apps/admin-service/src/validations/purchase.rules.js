...
const SORT_FIELDS = ['id', 'uuid', 'purchaseDate', 'totalAmount', 'pointsEarned', 'status', 'createdAt'];
...
export default {
  create: z.object({
    product_uuid: uuidParam,
    quantity: z.coerce.number().int({ message: 'validation.numeric' }).positive({ message: 'validation.numeric' }).max(1000, { message: 'validation.max' }),
  }),
  list: z.object({
